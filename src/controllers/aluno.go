...
func (h *AlunosHandler) GetByCPF(c *gin.Context) {
	cpf := c.Param("cpf")
	aluno, err := h.repo.GetByCPF(c.Request.Context(), cpf)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"error": "aluno não encontrado"})
		} else {
...
func (h *AlunosHandler) Create(c *gin.Context) {
	...
	normalizedCPF, err := helpers.NormalizeString(aluno.CPF)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	aluno.CPF = normalizedCPF

	if err := models.AlunoValidator(&aluno); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "esquema inválido"})
		return
	}

	created, err := h.repo.Create(ctx, aluno)
	if err != nil {
